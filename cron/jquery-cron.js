...
(function($) {
    ...
    var str_opt_dow = "";
    var days = ["воскресенье", "понедельник", "вторник", "среду", "четверг",
                "пятницу", "субботу"];
    ...
    // options for period
    var str_opt_period = "";
    var periods = ["минуту", "час", "день", "неделю", "месяц", "год"];
    ...
    var methods = {
        init : function(opts) {
            ...
            block["period"] = $("<span class='cron-period'>"
                    + "Каждый(ую) <select name='cron-period'>" + custom_periods
            ...
            block["dow"] = $("<span class='cron-block cron-block-dow'>"
                    + " в <select name='cron-dow' multiple>" + str_opt_dow
                    + "</select> </span>")
                .appendTo(this)
                .data("root", this);
            ...
            block["time"] = $("<span class='cron-block cron-block-time'>"
                    + " в <select name='cron-time-hour' class='cron-time-hour'>" + str_opt_hid
        ...
        value : function(cron_str) {
            ...
            if (defined(o.customValues) && o.customValues.hasOwnProperty(t)) {
                ...
                for (var i = 0; i < targets.length; i++) {
                    var tgt = targets[i];
                    if (tgt == "time") {
                        var btgt = block[tgt].find("select.cron-time-hour").val(v["час"]);
